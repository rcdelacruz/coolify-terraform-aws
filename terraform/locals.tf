...
locals {
  ...
  # User data scripts (base64 encoded) - Using minimal versions to stay under 16KB AWS limit
  control_user_data = base64encode(templatefile("${path.module}/control_user_data_minimal.sh", local.control_user_data_vars))
  remote_user_data  = base64encode(templatefile("${path.module}/remote_user_data_minimal.sh", local.remote_user_data_vars))
